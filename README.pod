...
=head3 borderStyle,borderColour  

Optionally set to draw a border around chart.
Valid styles are "simple", "double", "shadow", "thin" and "thick";

=head3 $canvas->set($x, $y, [$value])
...
The line C<use open ":std", ":encoding(UTF-8)";> in this version allows 
the encoding to be handled for printing to the terminal using
C<$canvas-E<gt>draw($row,$column)> (see below)


=head3 $canvas->draw(), $canvas->draw($row,$column)

Draws the canvas at specific row and column in ANSI terminals.
If C<$canvas-E<gt>{borderStyle}> is specified, then also prints a border

=begin HTML

<p><img src="https://github.com/saiftynet/term-drawille/edit/master/spinningcube.gif"
width="500" height="250" alt="spinningcube" />Spinning Cube</p>
